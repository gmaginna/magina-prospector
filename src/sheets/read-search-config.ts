...
export async function readSearchConfig(sheets: SheetsClient): Promise<SearchConfig[]> {
  const rows = await sheets.read(`${quoteSheet("Busca")}!A1:Z1000`);
  ...
  const columns = {
    ...
    lastRun: indexOfHeader(headers, ["ultima execucao"]), nextRun: indexOfHeader(headers, ["proxima execucao"]),
  };
  for (const key of ["active", "niche", "city", "priority", "lastRun", "nextRun"] as const) if (columns[key] < 0) throw new Error(`Coluna obrigatória ausente na aba Busca: ${key}`);
  const get = (row: string[], column: number, fallback = "") => column >= 0 ? (row[column] ?? fallback).trim() : fallback;
  return rows.slice(headerIndex + 1).flatMap((row, index): SearchConfig[] => {
    ...
    return [{
      niche, city, state, maxResults: Number.isFinite(max) && max > 0 ? Math.min(max, 100) : 20,
      highValue, priority, notes: get(row, columns.notes),
